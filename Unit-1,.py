...
class TaxiDispatchMDP:
    def __init__(self, grid_size=5, pickups=None, gamma=0.9):
        self.grid_size = grid_size
        self.pickups = pickups if pickups else [(4,4), (2,3)]
        self.gamma = gamma
        self.states = [(r,c) for r in range(grid_size) for c in range(grid_size)]
        self.actions = ["up","down","left","right"]

    def reward(self, state):
        if state in self.pickups:
            return 10
        else:
            return -1
...
print("\nOptimal Policy (best action at each location):")
for r in range(env.grid_size):
